...
require 'koneksi.php';
$dbarang = query("SELECT * FROM anggota");

if (isset($_POST['cari'])) {
  $dbarang = caria($_POST['keyword']);
}
        ...
        <?php if (empty($dbarang)) : ?>
          <tr>
            <td colspan="4" class="alert alert-danger text-center" role="alert">
              <p><b>Anggota tidak ditemukan</b></p>
            </td>
          </tr>
        <?php endif; ?>
        <tbody>
          ...
          <?php foreach ($dbarang as $brg) : ?>
            <tr>
              ...
              <td><?php echo $brg['notelp']; ?></td>
              <td><?php echo $brg['alamat']; ?></td>
              <td>
                <li">
                  <a href="edit_anggota.php?id=<?= $brg['id'];  ?>" class="btn btn-warning" role="button">Edit</a> |
